...
function renderDetail(){queueMicrotask(refreshPrices);const id=location.hash.split('/')[1];const p=products.find(p=>p.id===id);if(!p){if(!catalogueReady){app.innerHTML=catalogueLoading('THE COLLECTION');return}const current=window.tradeCatalog.find(x=>x.id===id);if(current&&catalogueReady){app.innerHTML=managedPublicDetail(current);return}location.hash='collection';return}if(detailModel!==id){detailModel=id;variantIndex=0;viewIndex=0;detailQty=1;lensIndex=-1}const v=p.variants[variantIndex]||p.variants[0];if(viewIndex>=v.images.length)viewIndex=0;const im=v.images[viewIndex];const liveDescription=window.tradeCatalog.find(x=>x.id===id)?.description||'';document.querySelectorAll('nav a').forEach(a=>{a.classList.toggle('active',a.dataset.page==='collection');if(a.dataset.page==='collection')a.setAttribute('aria-current','page');else a.removeAttribute('aria-current')});document.querySelector('.sidebar').classList.remove('open');document.querySelector('.mobile-toggle').setAttribute('aria-expanded','false');document.title=`${id} · ${v.name} — DOOK Denmark`;app.innerHTML=`<div class="detail-breadcrumb"><a href="#${returnPage}">← ${returnPage==='partners'?t('common.viewPartnerCollection'):t('common.theCollection')}</a><span>${t('detail.framesSlash',{id})}</span></div><section class="product-detail"><div class="detail-gallery"><div class="gallery-stage"><span class="gallery-mark">DOOK / ${id}</span><button class="zoom-image" aria-label="${t('detail.enlargeProductAria',{id})}" data-zoom><span class="detail-photo"><img class="detail-main-image" src="${im.src}" alt="${id} in ${v.name}, ${im.label}"><span class="detail-dook-layer detail-dook-left" hidden aria-hidden="true"><img alt=""></span><span class="detail-dook-layer detail-dook-right" hidden aria-hidden="true"><img alt=""></span></span></button><span class="gallery-view">${im.label}</span><button class="zoom-trigger" data-zoom aria-label="${t('detail.enlargeAria')}">＋</button></div><div class="gallery-bottom"><div class="gallery-thumbs" aria-label="${t('detail.productViewsAria')}">${v.images.map((pic,i)=>`<button data-view="${i}" class="${i===viewIndex?'selected':''}" aria-pressed="${i===viewIndex}" aria-label="${t('detail.showAria',{label:pic.label})}"><img src="${pic.src}" alt="${pic.label}"></button>`).join('')}</div><span class="image-count">0${viewIndex+1} / 0${v.images.length}</span></div></div><div class="detail-info"><h1>${id}</h1><div class="detail-option"><div class="option-label"><span>${t('common.colourLabel')}</span><strong>${v.name}</strong></div><div class="colour-options" aria-label="${t('detail.chooseColourAria')}">${p.variants.map((c,i)=>`<button data-colour="${i}" style="--swatch:${c.hex}" class="${i===variantIndex?'selected':''}" aria-pressed="${i===variantIndex}" aria-label="${c.name}"><span></span></button>`).join('')}</div><span class="colour-code">${id} / ${v.code}</span></div>${lensOptions(id)}${detailPriceHTML()}${id==='A012'?`<div class="detail-size"><span>${t('common.lensSize')}</span><span class="size-value">50 mm</span></div>`:''}${returnPage==='partners'?`<div class="detail-cta"><div class="detail-quantity"><button data-detail-qty="-1" aria-label="${t('detail.decreaseQty')}" ${detailQty===1?'disabled':''}>−</button><output>${detailQty}</output><button data-detail-qty="1" aria-label="${t('detail.increaseQty')}" ${detailQty===100?'disabled':''}>+</button></div><button class="pill" data-detail-add>${t('detail.addToEnquiry')} <span></span></button></div>`:''}<p class="detail-support">${returnPage==='partners'?t('detail.supportPartner'):t('detail.supportPublic')}</p><div class="detail-facts"><span>${t('detail.designedInDenmark')}</span><span>${t('common.acetateFrame')}</span></div><details class="detail-accordion"><summary>${t('detail.accordionSummary')}<span>+</span></summary><p>${t('detail.accordionCopy')}</p>${returnPage==='partners'?`<button class="text-button" data-enquire>${t('detail.askMatchingDooks')} </button>`:''}</details></div></section><section class="section about-frame"><div class="section-title"><div><h2>${t('detail.aboutFrame')}</h2></div></div><p>${escapeHTML(liveDescription||t('detail.aboutFrameDefault'))}</p></section>`+footer();updateGalleryImage();for(const variant of p.variants)for(const photo of variant.images){const preload=new Image();preload.src=photo.src}}
...
document.addEventListener('click',e=>{
 ...
 if(e.target.closest('[data-enquire]')){const p=products.find(p=>p.id===detailModel),v=p.variants[variantIndex];contactDraft=t('detail.enquireDraft',{id:p.id,name:v.name,code:v.code,sizeNote:p.id==='A012'?t('detail.a012SizeNote'):'',lensDesc:lensDescription()});location.hash='contact'}
