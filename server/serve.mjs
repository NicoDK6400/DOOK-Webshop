...
async function serveStatic(request){
 ...
 try{
  ...
  return new Response(bytes,{headers:{'Content-Type':MIME[extname(path)]||'application/octet-stream','Cache-Control':hashed?'public, max-age=31536000, immutable':'public, max-age=3600'}});
 }catch{return new Response('Not found',{status:404})}
}

const env={DB:openDB(),MEDIA:openMedia(),ASSETS:{fetch:serveStatic},OWNER_ACTIVATION_TOKEN:process.env.OWNER_ACTIVATION_TOKEN};
